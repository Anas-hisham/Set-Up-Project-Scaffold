...
<template>
  <div class="flex justify-between items-center border px-4 py-2">
    ...
    <div class="flex items-center gap-2">
      <img
        v-if="match[matchIndex === 0 ? 'firstMatchImages' : 'secondMatchImages'][`${side}Team${type === 'logo' ? 'Logo' : 'Flag'}`]"
        :src="match[matchIndex === 0 ? 'firstMatchImages' : 'secondMatchImages'][`${side}Team${type === 'logo' ? 'Logo' : 'Flag'}`]"
        class="w-8 h-8 object-cover cursor-pointer"
        @click="openImageDialog(matchIndex, `${side}Team${type === 'logo' ? 'Logo' : 'Flag'}`)"
